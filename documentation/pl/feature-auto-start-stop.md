...
**Uwaga:** Gdy funkcja autoSTART/autoSTOP wyzwala automatyczne zatrzymanie, wyłącza wyłącznie urządzenie źródłowe; wyeksponowany termostat _VTherm_ zachowuje żądany przez użytkownika `hvac_mode` (np. `heat` lub `cool`), a jego `hvac_action` przechodzi na `off`, wskazując, że urządzenie jest obecnie nieaktywne. Funkcja ponownie uruchomi urządzenie źródłowe tylko wtedy, gdy zostało wcześniej wyłączone przez tę samą funkcję. Zapobiega to niepożądanym lub niespodziewanym aktywacjom. To, czy automatyczne zatrzymanie jest obecnie aktywne, można sprawdzić za pomocą atrybutu `is_auto_stop_detected` (w sekcji `auto_start_stop_manager` atrybutów _VTherm_) lub nasłuchując zdarzenia `versatile_thermostat_auto_start_stop_event`. Naturalnie, ten stan automatycznego zatrzymania jest zachowany nawet po ponownym uruchomieniu Home Assistanta.
...
> 3. Jeśli korzystasz z karty interfejsu Versatile Thermostat (patrz: [tutaj](additions.md#better-with-the-versatile-thermostat-ui-card)), na karcie bezpośrednio widoczne jest pole wyłączenia autoSTART/autoSTOP, a termostat _VTherm_, którego urządzenie źródłowe zostało zatrzymane przez tę funkcję, jest oznaczony ikoną ![auto-start/stop icon](images/auto-start-stop-icon.png).
