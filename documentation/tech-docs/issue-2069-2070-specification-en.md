...
| Version          | 1.3 (v1.1: QC-001/QC-002 closure; v1.2: integration of the `check_initial_state` startup defect; v1.3: final documentation review — `_has_valve_control` guard, fixed cross-references)                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Status           | Converged with the DESIGN-2069-2070 v1.3 design                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Date             | 2026-09-24 (v1.1: QC-001/QC-002 closure; v1.2: integration of the `check_initial_state` startup defect raised by the implementation review; v1.3: final documentation review)                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Owner            | versatile_thermostat maintainer                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Sources analyzed | `documentation/tech-docs/issue-2069-2070.md` (including the 2026-09-24 addendum — review of the current implementation), `documentation/tech-docs/issue-2069-2070-en.md`, `documentation/tech-docs/issue-2069-2070-design.md` (DESIGN-2069-2070 v1.3), `custom_components/versatile_thermostat/opening_degree_algorithm.py` (`OpeningClosingDegreeCalculation`), `custom_components/versatile_thermostat/underlyings.py` (`UnderlyingValve`, `UnderlyingValveRegulation`), `custom_components/versatile_thermostat/config_schema.py` (reviewed, not reread line by line), GitHub issues #2069 (PR) and #2070 |
...
- **FR-006** — During `check_initial_state` (startup or reload) of an `over_valve` VTherm, the system shall return a valve with no demand to the effective `100 - max_closing_degree` floor (constrained to the entity limits), rather than systematically to the minimum of the `number` entity. This closure shall be preceded, where relevant, by the synchronization required by FR-010.
...
- **FR-009** — User documentation of the behavior (floor, activity detection, monotonicity) shall be published equivalently in the five localized guides (`en`, `fr`, `cs`, `de`, `pl` — `documentation/*/over-valve.md`) **in the same release as the fix**. *(QC-002 decision ratified — see DESIGN-2069-2070 v1.3, §5.5 "M4 — Documentation".)*
- **FR-010** — During the first `check_initial_state` of an `over_valve` VTherm (startup or reload), if a TPI demand is already available on the thermostat side (`valve_open_percent`), the system shall synchronize this demand into the stored raw demand and convert it into an effective command through the same algorithm as the normal command, **before** evaluating the activity predicates (`should_device_be_active` / `is_device_active`) and before any closing or catch-up decision. Consequently: (a) a valve with no demand is indeed closed to the floor (FR-006, BR-004); (b) a valve with an actual demand is not closed to the floor and undergoes the catch-up towards the command corresponding to the demand (CA-C2). *(Integrates the blocking defect raised by the addendum of the 2026-09-24 review report; aligned with DESIGN-2069-2070 v1.3 §5.4.)*

### 5. Business rules

- **BR-001** — For `over_valve`: a valve is considered active if and only if the raw TPI demand is `> 0` and `>= opening_threshold_degree` (current algorithm semantics). Exceptions: zero raw demand → always inactive; unknown raw demand (before the first cycle) → inactive; configurations where **none** of the four valve-control parameters is configured away from its default (`opening_threshold_degree` at 0, `max_closing_degree` at 100, `min/max_opening_degrees` absent), i.e. when the `_has_valve_control` guard is false → existing behavior (comparison of the effective command with the entity minimum) is preserved. Priority: high.
...
| B4  | Raw demand 30 (threshold equality, > 0: `opening_threshold_degree=30`, valve at the floor or converted)                                     | active: `should_device_be_active` is `True` (equality included via `>=`), published state `heating` if the valve is physically open beyond the floor | Yes                    |
| B5  | After `turn_off`: stored raw demand reset to 0 (`_raw_percent_open = 0`), command at the effective floor, `should_device_be_active` `False` | `idle`/`off`                                                                                                                                         | No                     |

#### CA-C — `check_initial_state` (`over_valve`, FR-006, FR-010, BR-004)

- C1: `over_valve` VTherm with no demand, valve open at 60, effective floor 40, entity min 0 → after `check_initial_state`, the valve is at 40 (floor), not 0; the stored raw demand is reset to 0.
- C2 (catch-up with actual demand): VTherm with an already available positive actual TPI demand (≥ threshold), valve closed to the floor → during the first `check_initial_state`, the demand is first synchronized into the stored raw demand and converted into an effective command (FR-010), then the catch-up sends the command corresponding to the demand: the valve is **not** closed to the floor, it is listed as active and the published state is consistent (`heating`).
- C3 (closing then catch-up): VTherm with an actual demand but a valve physically in a state inconsistent with the demand (for example open at the entity minimum value, with no stored demand) → after synchronization (FR-010), the closing or catch-up decision is taken on the synchronized demand and the actual physical state, not on uninitialized startup values.

#### CA-D — `over_climate` valve regulation (FR-007)

- D1: with A2 parameters, any demand sent through `UnderlyingValveRegulation.send_percent_open` (including threshold crossing — demands 29 then 31 — and zero demand) produces an `opening_degree >= 40`; `closing_degree` is the expected complement to 100; the predicates of `UnderlyingValveRegulation` are unchanged (BR-005, CA-D2).
...
- E3: for an `over_valve` configuration where **none** of the four valve-control parameters is configured away from its default (`opening_threshold_degree` at 0, `max_closing_degree` at 100, `min/max_opening_degrees` absent) — the condition under which the `_has_valve_control` guard is false —, activity predicates retain prior behavior (no regression of default configurations): with an effective command greater than the entity minimum, `should_device_be_active` and `is_device_active` return `True` through the fallback guard. The fallback guard shall **not** be promised when any single one of these parameters is configured away from its default (e.g. `max_closing_degree != 100` alone): in that case, `_has_valve_control` is true and activity is evaluated per FR-003 (comparison on the defined scales), with no fallback.

#### CA-F — Aggregated integration (`over_valve`, FR-003, FR-004, FR-010)

- F1 (aggregation of B1): situation B1 (raw demand 0, valve at the floor) → the VTherm publishes an `hvac_action` consistent with the absence of demand (`idle`/`off`), the valve does not appear in the active underlying devices (`device_actives`), and the central boiler is not activated by this VTherm.
- F2 (aggregation of B2): situation B2 (raw demand 100, threshold 60 > max 50, valve at 50) → the VTherm publishes `heating`, the valve appears in `device_actives`, and the central boiler can be activated by this VTherm.
- F3 (aggregation of C2): at startup with actual demand and effective catch-up (C2), the aggregated states (`hvac_action`, `device_actives`, central boiler) reflect the demand from the first cycle following `check_initial_state`, with no inconsistency window.
...
- **QC-001 (resolved — DESIGN-2069-2070 v1.3, §5.3, QC-001 decision)**: a single scale *per predicate*. Desired activity (`should_device_be_active`) is evaluated on raw TPI demand compared to `opening_threshold_degree`; observed activity (`is_device_active`) is evaluated on actual physical opening compared to the effective physical floor `max(100 - max_closing_degree, entity minimum)`. Alternative 2 (converting the threshold to a physical threshold per valve) is discarded: it would require duplicating the conversion formula (including entity clamping) for a threshold that is never sent, with a risk of divergence between sending and comparison. FR-003, FR-004 and BR-001 reflect this decision; BR-001 incorporates the compatibility guard exceptions (default configurations, unknown demand).
- **QC-002 (resolved — DESIGN-2069-2070 v1.3, §5.5, QC-002 decision)**: the five localized documentations (`en`, `fr`, `cs`, `de`, `pl`) are delivered in the same PR as the fix, in order to avoid a documentation inconsistency window between published versions. FR-009 and CA-E2 reflect this decision.

**Open questions (non-blocking for development):**
- **QC-003 (open question, inherited from the design)**: in the degenerate case where `max_opening_degrees < 100 - max_closing_degree` (the algorithmic floor exceeds the configured maximum), clamping to the floor may produce a command greater than `max_opening_degree`; clamping to the limits of the `number` entity remains the physical arbiter. This case, accepted by the current configuration flow, shall be observed during implementation; the possible addition of a non-blocking warning in the configuration flow remains a future enhancement (section 9).

**Convergence traceability (v1.3):** this v1.3 specification and DESIGN-2069-2070 v1.3 cover the same scope (`over_valve` + shared `OpeningClosingDegreeCalculation` algorithm for `over_climate` valve regulation); all requirements FR-001 to FR-010, rules BR-001 to BR-005 and criteria CA-A to CA-F are covered by the design or by this convergence addendum; the startup defect raised by the report addendum (synchronization of the TPI demand at the first `check_initial_state`) is integrated through FR-010 and traced in CA-C2/CA-C3 and CA-F3; no contradiction between the specification, the design and the review report was identified; questions QC-001 and QC-002 are closed, FR-010/CA-C2 address the blocking finding of the addendum, and the remaining questions (QC-003) are declared non-blocking.

### 11. Version history

- Version 1.3 (2026-09-24) — final documentation review: E3 and the BR-001 exception reworded around the actual `_has_valve_control` guard (complete default condition: `opening_threshold_degree` at 0, `max_closing_degree` at 100, `min/max_opening_degrees` absent; no fallback when any single parameter is away from default); broken references to identifiers "D1"/"D2"/"QO-1" replaced with references to the actual sections of DESIGN-2069-2070 v1.3 (§5.3, §5.5); CA-D1 now refers to CA-D2. No requirement added, removed or weakened.
- Version 1.2 (2026-09-24) — integration of the startup defect raised by the review report addendum (synchronization of the TPI demand at the first `check_initial_state` before predicate evaluation/catch-up) through the new FR-010; FR-006 clarified; CA-C2 clarified (catch-up with actual demand) and new CA-C3; new criterion B5 (reset in `turn_off`); B4 clarified (threshold equality); E3 clarified; CA-D1 clarified (threshold crossing through `UnderlyingValveRegulation.send_percent_open`); new CA-F section (aggregated integration `hvac_action` / `device_actives` / central boiler); design reference corrected from DESIGN-2069-2070 v1.0 to v1.2. No requirement removed or weakened; existing scope and decisions preserved.
- Version 1.1 (2026-09-24) — convergence cycle with the design: QC-001 closed (option 1) and reflected in FR-003/FR-004/BR-001/BR-004; QC-002 closed and reflected in FR-009/CA-E2; FR-003 clarified as a single scale *per predicate*; compatibility guard for default configurations tracked by BR-001 and the new CA-E3 criterion; QC-003 question created (degenerate case `max_opening_degrees < 100 - max_closing_degree`, non-blocking). No requirement removed or weakened.
