...
*   `model_confidence` : Indice de confiance (0.0 à 1.0) sur la qualité des réglages.
...
**Principe :** Le service utilise les données des **X derniers jours** d'historique (via le composant `recorder` de Home Assistant) pour trouver la meilleure corrélation entre la puissance utilisée et le changement de température, en ignorant les facteurs extérieurs (capacité adiabatique). Il effectue une régression linéaire sur les cycles considérés comme étant à pleine puissance.
> **Prise en compte du Délai de Désactivation Minimal** : L'analyse des cycles à pleine puissance prend en compte le `minimal_deactivation_delay`. Si la puissance TPI calculée est supérieure à la puissance minimale requise pour que le relais reste constamment **ON** (en raison de la contrainte du délai de désactivation), le cycle est inclus dans la régression pour garantir une estimation précise de la capacité.
...
> **Résultat** : La valeur de la Capacité Maximale (`max_capacity_heat`/`cool`) est mise à jour dans les attributs du capteur d'état d'apprentissage.
>
> Le service retourne également les informations suivantes pour analyser la qualité de la calibration :
> *   **`capacity`** : La capacité thermique calculée (en °C/h).
> *   **`reliability`** : Indice de fiabilité (en %) de la calibration, basé sur la qualité statistique de la régression.
> *   **`k_ext`** : Estimation du coefficient extérieur (correspondant à la pente de la régression).
> *   **`min_power_threshold`** : Seuil de puissance utilisé pour détecter les cycles à 100%.
