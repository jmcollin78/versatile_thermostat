...
- ``versatile_thermostat_hvac_mode_event`` : le thermostat est allumé ou éteint. Cet évènement est aussi diffusé au démarrage du thermostat. Cet évènement n'est plus déclenché lorsque la fonction auto-start/stop arrête ou relance l'équipement sous-jacent : utilisez plutôt ``versatile_thermostat_auto_start_stop_event`` et l'attribut ``is_auto_stop_detected``
...
- ``versatile_thermostat_auto_start_stop_event`` : un évènement indiquant un arrêt ou une relance de l'équipement sous-jacent fait par la fonction d'auto-start/stop. Le ``hvac_mode`` du thermostat exposé n'est pas modifié : seul l'équipement sous-jacent est arrêté ou relancé
...
| ``hvac_action``                                 | L'action en cours d'exécution par le réchauffeur. Peut être inactif, chauffage, refroidissement, off (par exemple lorsque la fonction auto-start/stop a arrêté l'équipement sous-jacent)                            |
...
| ``hvac_off_reason``                             | Indique la raison de l'arrêt (hvac_off) du VTherm. Ce peut être Window ou Manuel. La fonction auto-start/stop ne renseigne jamais cet attribut (elle arrête seulement l'équipement sous-jacent, sans faire passer le `hvac_mode` du VTherm à off) ; utilisez `is_auto_stop_detected` (cf. section `auto_start_stop_manager` ci-dessous) à la place                       |
...
| ``is_auto_stop_detected``                       | `true` si l'équipement sous-jacent a été arrêté automatiquement par la fonction auto-start/stop. C'est l'attribut de référence pour détecter un arrêt automatique : le `hvac_mode` du VTherm n'est pas modifié, seul son `hvac_action` passe à `off`                       |
