...
Hinweis: Wenn die Auto-Start/Stop-Funktion einen automatischen Stopp auslöst, schaltet sie nur das zugrunde liegende Gerät aus; das angezeigte _VTherm_ behält den von Ihnen angeforderten `hvac_mode` (z. B. `heat` oder `cool`) bei, und sein `hvac_action` wechselt zu `off`, um anzuzeigen, dass das Gerät derzeit inaktiv ist. Die Funktion schaltet das zugrunde liegende Gerät nur dann wieder ein, wenn es durch diese Funktion selbst ausgeschaltet wurde. Dies verhindert ungewollte oder unerwartete Aktivierungen. Ob gerade ein automatischer Stopp aktiv ist, können Sie über das Attribut `is_auto_stop_detected` (im Abschnitt `auto_start_stop_manager` der _VTherm_-Attribute) oder durch Abhören des Ereignisses `versatile_thermostat_auto_start_stop_event` prüfen. Selbstverständlich bleibt dieser automatisch gestoppte Zustand auch nach einem Neustart von Home Assistant erhalten.
...
> 3. Wenn Sie die Versatile Thermostat UI-Karte verwenden (siehe [hier](additions.md#better-with-the-versatile-thermostat-ui-card)), ist ein Kontrollkästchen direkt auf der Karte sichtbar, um Auto-Start/Stopp zu deaktivieren, und ein _VTherm_, dessen zugrunde liegendes Gerät durch Auto-Start/Stopp gestoppt wurde, wird durch dieses Symbol angezeigt: ![auto-start/stop icon](images/auto-start-stop-icon.png).