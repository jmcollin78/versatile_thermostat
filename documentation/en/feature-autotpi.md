...
*   **Cooling Rate** (`auto_tpi_cooling_rate`): Target rate of temperature decrease in °C/h. ([see rates Configuration](#heating-cooling-rate-configuration))
    ...
    *Note: You don’t necessarily want to use the maximum heating/cooling rate. You can perfectly well use a lower value depending on the heating/AC system sizing.
...
**Principle:** The service uses the data from the **last X days** of history (via the Home Assistant `recorder` component) to find the best correlation between the power used and the temperature change, ignoring external factors (adiabatic capacity). It performs a linear regression on cycles considered to be at full power.
...
> **Result** : The Maximum Capacity value (`max_capacity_heat`/`cool`) is updated in the learning state sensor attributes.
>
> The service also returns the following information to analyze the calibration quality:
> *   **`capacity`**: The calculated thermal capacity (in °C/h).
> *   **`reliability`**: Reliability index (in %) of the calibration, based on the statistical quality of the regression.
> *   **`k_ext`**: Estimation of the outdoor coefficient (corresponding to the slope of the regression).
> *   **`min_power_threshold`**: Power threshold used to detect 100% cycles.
