...
Note: When it auto-stops, the auto-start/stop function only turns off the underlying device(s); the exposed _VTherm_ keeps the `hvac_mode` you requested (e.g. `heat` or `cool`), and its `hvac_action` switches to `off` to reflect that the equipment is currently inactive. The function will only turn the underlying device(s) back on if they were stopped by this function itself. This prevents unwanted or unexpected activations. You can check whether an auto-stop is currently active via the `is_auto_stop_detected` attribute (in the `auto_start_stop_manager` section of the _VTherm_'s attributes) or by listening to the `versatile_thermostat_auto_start_stop_event` event. Naturally, this auto-stopped state is preserved even after a Home Assistant restart.
...
> 3. If you use the Versatile Thermostat UI card (see [here](additions.md#better-with-the-versatile-thermostat-ui-card)), a checkbox is directly visible on the card to disable auto-start/stop, and a _VTherm_ whose underlying device has been stopped by auto-start/stop is indicated by the icon: ![auto-start/stop icon](images/auto-start-stop-icon.png).