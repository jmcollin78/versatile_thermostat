...
- ``versatile_thermostat_hvac_mode_event``: the thermostat is turned on or off. This event is also broadcast at the thermostat's startup. This event is no longer fired when the auto-start/stop function stops or restarts the underlying device(s): use ``versatile_thermostat_auto_start_stop_event`` and the ``is_auto_stop_detected`` attribute instead
...
- ``versatile_thermostat_auto_start_stop_event``: an event indicating a stop or restart of the underlying device(s) made by the auto-start/stop function. The exposed thermostat's ``hvac_mode`` is not changed; only the underlying device(s) are stopped or restarted
...
| ``hvac_action``                                 | The action currently being performed by the heater. Can be idle, heating, cooling, off (for example when the auto-start/stop function has stopped the underlying device(s))                                    |
...
| ``hvac_off_reason``                             | Indicates the reason for the VTherm shutdown (hvac_off). Can be Window or Manual. The auto-start/stop function never sets this attribute (it only stops the underlying device(s), it doesn't switch the VTherm's `hvac_mode` to off); use `is_auto_stop_detected` (see `auto_start_stop_manager` section below) instead                       |
...
| ``is_auto_stop_detected``                       | `true` if the underlying device(s) have been automatically stopped by the auto-start/stop function. This is the source of truth to detect an auto-stop: the VTherm's `hvac_mode` is not changed, only its `hvac_action` switches to `off`                       |
