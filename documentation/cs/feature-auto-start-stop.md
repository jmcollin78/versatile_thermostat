...
Poznámka: Když dojde k automatickému zastavení, funkce automatického spuštění/zastavení vypne pouze podkladové zařízení; zobrazený _VTherm_ si zachová `hvac_mode`, který jste požadovali (např. `heat` nebo `cool`), a jeho `hvac_action` přejde na `off`, což znamená, že zařízení je momentálně neaktivní. Funkce znovu zapne podkladové zařízení pouze pokud bylo vypnuto touto funkcí. To zabraňuje nežádoucím nebo neočekávaným aktivacím. Zda je automatické zastavení aktuálně aktivní, můžete zjistit pomocí atributu `is_auto_stop_detected` (v sekci `auto_start_stop_manager` atributů _VTherm_) nebo sledováním události `versatile_thermostat_auto_start_stop_event`. Přirozeně je tento stav automatického zastavení zachován i po restartu Home Assistant.
...
> 3. Pokud používáte UI kartu Versatile Thermostat (viz [zde](additions.md#better-with-the-versatile-thermostat-ui-card)), zaškrtávací políčko je přímo viditelné na kartě pro deaktivaci automatického spuštění/zastavení a _VTherm_, jehož podkladové zařízení bylo zastaveno automatickým spuštěním/zastavením, je označen ikonou: ![ikona automatického spuštění/zastavení](images/auto-start-stop-icon.png).
