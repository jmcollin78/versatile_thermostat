...
| ``hvac_off_reason``                             | Důvod vypnutí VTherm (`hvac_off`). Může být Window, Auto-start/stop, Manual nebo Sleep                                                                                                                       |
| ``hvac_mode_reason``                            | Důvod aktuálního režimu VTherm (`hvac_mode`). Může být Window, Auto-start/stop (upřesněno podle režimu zastavení: off, fan-only nebo dry), Central mode, Safety, Manual nebo Sleep                           |
...
| ``is_sleeping``                                 | Indikuje, že VTherm je ve sleep režimu (platí pro `over_climate` s přímým řízením ventilu a `over_valve`)                                                                                                    |
...
| ``auto_start_stop_stop_mode``                   | Režim použitý při automatickém zastavení. Může být `off`, `fan_only` nebo `dry`                                                                                                                              |
