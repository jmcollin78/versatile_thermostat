...
async def test_auto_tpi_convergence_simulation(mock_hass, mock_store, mock_config_entry):
    ...
    prop_algo = PropAlgorithm(
        function_type="tpi", tpi_coef_int=0.3, tpi_coef_ext=0.01, cycle_min=10, minimal_activation_delay=0, minimal_deactivation_delay=0, vtherm_entity_id="climate.sim_test"
    )
    ...
    with patch("custom_components.versatile_thermostat.auto_tpi_manager.datetime") as mock_datetime, patch(
        "custom_components.versatile_thermostat.auto_tpi_manager.async_call_later"
    ) as mock_call_later:

        mock_datetime.now.side_effect = lambda: sim_time

        async def data_provider():
            await manager.update(room_temp=model.room_temp, ext_temp=model.ext_temp, target_temp=target_temp, hvac_mode="heat", is_overpowering_detected=False)
            ...
            prop_algo.calculate(target_temp=target_temp, current_temp=model.room_temp, ext_current_temp=model.ext_temp, slope=None, hvac_mode="heat")
            on_percent = prop_algo.on_percent

            return {"on_time_sec": on_percent * 600, "off_time_sec": (1 - on_percent) * 600, "on_percent": on_percent, "hvac_mode": "heat"}
        ...
        for i in range(50):
            ...
            print(
                f"{i+1:<5} | {model.room_temp:<8.3f} | {model.ext_temp:<8.1f} | {power:<8.2f} | {manager.state.coeff_indoor_heat:<8.3f} | {manager.state.coeff_outdoor_heat:<8.3f} | {status}"
            )
