...
async def test_load_data_existing(manager, mock_store):
    ...
    existing_state = AutoTpiState(coeff_indoor_heat=0.3, coeff_outdoor_heat=0.05, total_cycles=100, coeff_indoor_autolearn=50, coeff_outdoor_autolearn=50)
...
async def test_update_state(manager):
    ...
    await manager.update(room_temp=20.0, ext_temp=5.0, target_temp=21.0, hvac_mode="heat")
...
async def test_cycle_lifecycle(manager):
    ...
    # 1. Start Cycle
    await manager.on_cycle_started(on_time_sec=150, off_time_sec=150, on_percent=0.5, hvac_mode="heat")
    ...
    # 2. Complete Cycle
    await manager.on_cycle_completed(on_time_sec=150, off_time_sec=150, hvac_mode="heat")
...
def mock_vtherm_for_capacity():
    ...
    manager.calculate_capacity_from_history = AsyncMock(return_value={"success": True, "capacity": 7.5, "kext": 0.015, "kint": 0.5})
...
async def test_service_calibrate_capacity(mock_vtherm_for_capacity):
    ...
    with patch("custom_components.versatile_thermostat.base_thermostat.history.get_significant_states") as mock_get_history, patch.object(
        vtherm, "_async_update_tpi_config_entry", new=AsyncMock()
