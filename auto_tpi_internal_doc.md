...
1.  **Démarrage** :
    Lorsque le thermostat passe en mode `HEAT` ou `COOL` (déterminé par le `hvac_mode` du VTherm), `BaseThermostat.update_states` appelle `manager.start_cycle_loop` (que l'apprentissage soit actif ou non).
...
*   **`confidence`** (0.0 - 1.0) : Fiabilité du modèle. Augmente avec le nombre de cycles, diminue avec les échecs.
...
*   **Nouveau Service de Calibration (`calibrate_capacity`)** :
    ...
    *   **Correction d'Inertie Thermique** : La capacité calculée via l'ordonnée à l'origine (Intercept) correspond à la capacité *effective* sur la durée du cycle. Une correction est appliquée pour compenser le temps de chauffe du radiateur (`heater_heating_time`).
        ...
        *   `Capacité Réelle = Capacité Effective / Facteur Efficacité`
    *   **Résultats et Métriques** : Le service retourne désormais des métriques enrichies pour l'analyse (au lieu du simple `r_squared`) :
        *   **`reliability`** : Une mesure de confiance en % basée sur l'erreur standard de l'ordonnée à l'origine (Intercept). Formule : `max(0, 100 * (1 - (2 * SE_intercept / Capacity)))`. Plus l'erreur standard est faible par rapport à la capacité, plus la fiabilité est élevée.
        *   **`k_ext`** : Correspond à l'opposé de la pente (`-slope`) de la régression linéaire. Cela fournit une estimation statistique du coefficient extérieur.
        *   **`min_power_threshold`** : Le seuil de puissance (ex: 0.95) utilisé pour filtrer les cycles saturés.
...
*   **Démarrage (`start_learning`)** : L'appel à `start_learning()` (ex: via le service `set_auto_tpi_mode(true)` ou initialisation) :
    *   Réinitialise les compteurs et les coefficients (sauf si fournis). La capacité calibrée est **conservée**.
