...
*   **Démarrage (`start_learning`)** : L'appel à `start_learning(reset_data, ...)` (ex: via le service `set_auto_tpi_mode`) :
    ...
    *   **Paramètre `reset_data`** (défaut: `True`) : Contrôle la réinitialisation des données d'apprentissage.
        ...
        *   **Note importante** : Si l'apprentissage était arrêté (`autolearn_enabled` à False), `reset_data` est forcé à `True` pour garantir que la nouvelle session démarre avec les coefficients configurés (ou par défaut) et non avec les résidus de la session précédente.
